...
class RoomData {
  ...
  const RoomData(
      {required this.players, required this.events, required this.gameState});
}
...
class Room {
  ...
  static Room createRoom({required Player player, required Game game}) {
    ...
    return Room(
        ...
        players: players,
        events: [],
        gameState: game.getInitialGameState(players: players));
  }
  ...
  static Room joinRoom(
    ...
    return Room(
        ...
        gameState: getGameStateFromEvents(
            game: game, players: players, events: events));
  }
  ...
  static Map<String, dynamic> getGameStateFromEvents(
      ...
      required List<Player> players,
      required List<Map<String, dynamic>> events}) {
    final gameState = game.getInitialGameState(players: players);
    for (final event in events) {
      game.processEvent(event: event, gameState: gameState, players: players);
    }
    return gameState;
  }
  ...
  void processEvent(Map<String, dynamic> event) {
    return game.processEvent(
        event: event, gameState: gameState, players: players);
  }

  void onPlayerJoin(Player player) {
    return game.onPlayerJoin(
        player: player, gameState: gameState, players: players);
  }
  ...
  Map<String, dynamic>? checkGameEnd() {
    return game.checkGameEnd(gameState: gameState, players: players);
  }
